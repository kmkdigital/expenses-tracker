                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ marginTop: 6 }}>
                          {sub.transactions.slice(0, 3).map(t => (
                            <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 12, color: 'var(--text-3)' }}>
                              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>{t.description}</span>
                              <span style={{ color: t.amount >= 0 ? 'var(--green)' : 'var(--red)', fontFamily: 'var(--font-display)' }}>
                                {t.amount >= 0 ? '+' : ''}{formatCurrency(t.amount)}
                              </span>
                            </div>
                          ))}
                          {sub.transactions.length > 3 && (
                            <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 4 }}>
                              +{sub.transactions.length - 3} more transactions
                            </div>
                          )}
                        </div>
                      </div>
